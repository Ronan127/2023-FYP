function [solution, remainingEquations] = mysolver(equations, solution)
arguments 
    equations = cell;
    solution = struct;
end
...
while isempty(equations) == 0
    ...
    for i = 1:length(equations)
        ...
        elseif length(vars) == 1
            ...
            if lhs(equations(i)) == rhs(equations(i))
                ...
                solution.(string(vars)) = double(rhs(tempeq));
                equations(i) = [];
    ...
    if timeout == 2
        ...
        [~, idx] = min(cellfun('size',vareqs,2));
        for i = idx
            combination = vareqs{i};
            for j = 1:length(vareqs)
